...
var (
	...
	jobName     = flag.String("jobName", "", "Job name the builder runs in")
...
func main() {
	...
	if *jobName == "" {
		log.Fatalf("Job Name must be provided.")
	}
	...
	if *mode == "trigger" {
		...
		slackbot.Trigger(ctx, projectId, *buildId, *webhook, *copyName, *copyTags, *copyTimeout, *jobName)
		return
	}
	...
	if *mode == "monitor" {
		...
		slackbot.Monitor(ctx, projectId, *buildId, *webhook, *jobName)
		return
	}
}
